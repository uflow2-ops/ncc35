...
let viewDate = new Date();
        ...
        let dailyCookieHistory = JSON.parse(localStorage.getItem('dailyCookieHistory_v1')) || {};
...
`;
        ...
        function renderCookieTrendChart() {
            ...
            dailyTotals.forEach((val, i) => {
                ...
                if (val > 0) {
                    ...
                    ctx.fillText(val + '개', x + barWidth / 2, y - 8);
                }
            });
        ...
        function renderClassStats() {
            ...
            // 현재 상위 5명 계산
            const currentScores = studentData.map(s => {
                const count = prevTotals[s.code] || 0;
                return { name: s.name, count: count };
            }).sort((a, b) => b.count - a.count);
            const top5Current = currentScores.slice(0, 5);
            ...
            const weeklyScores = studentData.map(s => {
                let weekTotal = 0;
                Object.entries(dailyCookieHistory).forEach(([date, data]) => {
                    if (date >= weekStartStr && data[s.code]) {
                        weekTotal += data[s.code];
                    }
                });
                return { name: s.name, count: weekTotal };
            }).sort((a, b) => b.count - a.count);
            const top5Weekly = weeklyScores.slice(0, 5);
            
            currentScores.forEach(s => {
                totalCookies += s.count;
            });
            ...
            let top5CurrentHtml = top5Current.map((s, i) => {
                const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : (i+1) + '.';
                return `<div style="padding:4px 0; ${i === 0 ? 'color:#ff9800; font-weight:bold;' : 'color:#666;'}">${medal} ${s.name}: <b>${s.count}</b>개</div>`;
            }).join('');
            
            let top5WeeklyHtml = top5Weekly.map((s, i) => {
                const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : (i+1) + '.';
                return `<div style="padding:4px 0; ${i === 0 ? 'color:#ff9800; font-weight:bold;' : 'color:#666;'}">${medal} ${s.name}: <b>${s.count}</b>개</div>`;
            }).join('');
            ...
            container.innerHTML = `
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-bottom:15px;">
                    <div style="padding:12px; background:#f3e5f5; border-radius:10px;">
                        <b style="font-size:1.2rem; color:#6a1b9a;">📊 전체 현황</b><br>
                        총 쿠키: <b style="color:#9c27b0; font-size:1.4rem;">${totalCookies}</b>개<br>
                        목표: <b style="color:#9c27b0;">${cookieGoal}개</b><br>
                        달성률: <b style="color:#9c27b0;">${goalProgress}%</b>
                    </div>
                    <div style="padding:12px; background:#e3f2fd; border-radius:10px;">
                        <b style="font-size:1.2rem; color:#1976d2;">📈 평균</b><br>
                        1인당: <b style="color:#1976d2; font-size:1.4rem;">${avgCookies}</b>개<br>
                        <small style="color:#666;">총 ${studentData.length}명</small>
                    </div>
                </div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div style="padding:12px; background:#fff9c4; border-radius:10px;">
                        <b style="font-size:1.1rem; color:#f57f17;">👑 현재 상위 5명</b><br>
                        ${top5CurrentHtml || '데이터 없음'}
                    </div>
                    <div style="padding:12px; background:#e8f5e9; border-radius:10px;">
                        <b style="font-size:1.1rem; color:#4caf50;">📅 이번 주 상위 5명</b><br>
                        ${top5WeeklyHtml || '데이터 없음'}
                    </div>
                </div>
            `;
        }
